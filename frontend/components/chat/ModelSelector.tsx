    ...
    <>
      <div className="relative">
        <button
          ...
          disabled={disabled}
          className={`
            flex items-center justify-between w-44 sm:w-52 px-4 py-3 text-sm
            border border-coral-200 dark:border-gray-600 rounded-2xl bg-white dark:bg-gray-800 hover:bg-coral-50 dark:hover:bg-gray-700
            focus:outline-none focus:ring-2 focus:ring-coral-400 focus:border-coral-400
            text-gray-900 dark:text-white
            ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"} shadow-lg hover:shadow-xl transition-all duration-200
          `}
        >
          ...
          <ChevronDown
            className={`h-4 w-4 text-coral-500 dark:text-gray-500 transition-transform flex-shrink-0 ${
              isOpen ? "rotate-180" : ""
            }`}
          />
        </button>
        ...
        {isOpen && (
          <div className="absolute top-full right-0 mt-2 w-[420px] bg-white dark:bg-gray-800 border border-coral-200 dark:border-gray-700 rounded-2xl shadow-2xl z-50 max-h-96 overflow-y-auto backdrop-blur-sm">
            <div className="p-3">
              {/* Free Models Section */}
              {models.filter((m) => m.isFree).length > 0 && (
                <>
                  <div className="px-4 py-3 text-xs font-bold text-coral-600 dark:text-gray-400 uppercase tracking-wider border-b border-coral-200 dark:border-gray-700">
                    Free Models
                  </div>
                  {models
                    .filter((m) => m.isFree)
                    .map((model) => (
                      <button
                        ...
                        className={`
                        w-full text-left p-4 rounded-2xl hover:bg-coral-50 dark:hover:bg-gray-700 transition-all duration-200
                        ${
                          selectedModel === model.id
                            ? "bg-coral-50 dark:bg-coral-900/20 border border-coral-200 dark:border-coral-700"
                            : ""
                        }
                      `}
                      >
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <div className="flex items-center space-x-1">
                            <span
                              className={`px-3 py-1 rounded-full text-xs border flex-shrink-0 font-medium ${getProviderColor(
                                model.provider,
                                model.isFree
                              )}`}
                            >
                            ...
                            <span className="px-3 py-1 rounded-full text-xs bg-green-100 text-green-800 border border-green-200 font-semibold">
                              FREE
                            </span>
                          </div>
                        </div>
                        ...
                        <p className="text-xs text-coral-500 dark:text-gray-500 font-medium">
              ...
              {models.filter((m) => !m.isFree).length > 0 && (
                <>
                  <div className="px-4 py-3 text-xs font-bold text-coral-600 dark:text-gray-400 uppercase tracking-wider border-b border-coral-200 dark:border-gray-700 mt-3">
                    Premium Models
                  </div>
                  {models
                    .filter((m) => !m.isFree)
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center space-x-2">
                            ...
                            <Key className="h-3 w-3 text-coral-500" />
                          </div>
                          <div className="flex items-center space-x-1">
                            <span
                              className={`px-3 py-1 rounded-full text-xs border flex-shrink-0 font-medium ${getProviderColor(
                            ...
                            <span className="px-3 py-1 rounded-full text-xs bg-orange-100 text-orange-800 border border-orange-200 font-semibold">
                              PREMIUM
                            </span>
                          </div>
                        </div>
